		...
		<>
			{
				daysArray.map((it) => {
					...
					return (
						<CeilOfCalendar key={it.format('MMDD')} it={it} today={today} holidayArr={holidayArr} isholiday={isHoliday} holidayName={holidayName} openFormHandler={openFormHandler} events={events.filter((filEv) => isEventInCurrentDay(filEv, it))} scrol={scrol} setScrol={setScrol} />
					)
				})
			}
		</>
